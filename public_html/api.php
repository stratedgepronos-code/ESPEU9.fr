...
case 'save_match_extras':
    ...
    try {
        ...
        // Nettoyer l'ancienne clé 'custom_X' si elle existe
        try { $db->prepare("DELETE FROM match_extras WHERE match_id=:old")->execute([':old'=>'custom_'.$matchId]); } catch (Exception $e) {}
        echo json_encode(['success'=>true]);
    ...
    try {
        ...
        if (!$fullSync) {
            ...
            echo json_encode([
                ...
                'matches_synced' => 0,
                'matches_total' => 0,
                'message' => "Classement mis à jour (les matchs en base ne sont pas modifiés). Import matchs FFBB : ajouter &full_sync=1 à l'URL (risque de doublons si IDs différents).",
            ]);
            break;
        }
